...
@Composable
fun TelaRegister(modifier: Modifier = Modifier, onRegisterClick: () -> Unit, onBackClick: () -> Unit){
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = modifier.fillMaxWidth()) {
        ...
        Button(modifier = Modifier.fillMaxWidth(), onClick = {
            scope.launch(Dispatchers.IO) {
                val newUser = Usuario(nome=login, senha=senha)
                usuarioDAO.adicionar(newUser, callback = { usuario ->
                    if (usuario != null) {
                        onRegisterClick()
                    } else {
